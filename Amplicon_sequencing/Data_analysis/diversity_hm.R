#Run diversity analysis for Hordeum murinum infected plants. 
...
plot.dir=paste(result.dir,"plot_paper_treatment_hm",sep = "/");dir.create(plot.dir)
...
png(paste(plot.dir,paste("p1",method,amplicon,"shannon.treatment.png",sep = "."),sep = "/"), 
    width = 850, height = 600, units = "px", pointsize = 15, bg = "white", res=300, type="cairo")
print(a) ; dev.off()
...
for ( i in unique(alpha$host.id)){
  for (j in unique(alpha[alpha$host.id==i, "time"])){
    if(j != "t0"){
      ...
      #test=summary(aov(shannon~treatment, data = sub))
      test=kruskal.test(shannon~treatment, data = sub)
      print(paste(i,j))
      print(test$p.value)
      #test=TukeyHSD(aov(shannon~treatment, data = sub))$time
...
if (amplicon == "ITS"){
  ...
  for (j in levels(met.2$time)[c(2,4,5,7)]){
    print(j)
    met.s=met.2[met.2$time==j,]
    mp.s=newMRexperiment(as(otu.z[, colnames(otu.z) %in% rownames(met.s)], "matrix"))
    mr=MRcounts( cumNorm( mp.s, p=cumNormStat( mp.s ) ),norm=TRUE, log=TRUE )
    set.seed(23171341)
    scaling=vegdist(t(mr), method = "bray", binary = F) #calculate distance
    per=adonis2(t(mr)~treatment, data = met.s, permutations = 1000)
    print(per)
...
otu.t0 = otu.t %>% group_by(host.id,treatment,time) %>% summarise(absab = sum(abs))
otu.t1 = otu.t %>% group_by(otu.t[,t],host.id,treatment,time) %>% summarise(absab = sum(abs)) 
